---
description: 디자인 토큰 일관성 가드 + 컴포넌트 인덱스 갱신 — 색·간격·둥글기·타이포 변경 발화·신규/변경 컴포넌트를 감지해 *부분 수정*인지 *시스템 변경*인지 자동 분기, 시스템 변경이면 명시적 동의. 디자인 토큰(색·간격·둥글기·타이포)이 영향받는 변경 또는 컴포넌트 신규/변경 시점에 — "이 버튼 좀 더 둥글게 해줘"·"색을 다른 걸로 바꿔줘"·"여백 좁혀줘"·"본문 글씨 키워줘" 같은 자연어.
model: inherit
---
...
`AskUserQuestion`으로 묻는다. "왜 묻는지" 1줄 + "잘 모르겠어요" 옵션 필수.
...
- *전체 시스템* → 토큰 정의 파일 수정
- 사용자가 한 사이클 3회 상한(결정 피로 최소화) 넘으면 기본값(부분)으로 자동 진행 + 사후 안내
...
- 동의 없는 광범위 변경 절대 X (명시 동의)

## 자가 점검

응답 직전 체크:
- [ ] 변경 분류 — *부분 수정*(이 화면만) vs *시스템 변경*(전 화면 영향) 중 *하나*
- [ ] 시스템 변경 분기는 명시 동의 받기
- [ ] 영향 범위 1줄 정량화 ("이 색은 다른 N곳에서도 쓰여요")
- [ ] 컴포넌트 인덱스(`CLAUDE.project.md` §components) 갱신 — 신규/변경 감지 시
- [ ] 슬롯 분량 *디자이너 한눈에 훑을 만함* 초과 시 archive 회전 (정본: `SCHEMA.md` §2.3)
