...
| 완전 빈 디렉토리(`Glob`로 파일 0개에 가까움) | "이 폴더에 가져올 코드를 먼저 넣어주세요 — 그 다음 '있는 거 분석해서 셋업해줘' 한 마디면 정리할게요" 안내 후 종료. *자동으로 `new-service` 권유 X* — 사용자가 이 Skill을 직접 부른 건 *기존 프로젝트 가져오기 의도*가 명확하니, 빈 폴더라고 새 디자인으로 우회시키지 X | — |
