...
추출은 *프로젝트 미학 슬롯에도 시드*한다 — `CLAUDE.project.md` 디자인 시스템 §철학 옆 슬롯들로:
- 추출한 폰트가 한글 친화(Pretendard/Spoqa/SUIT 등)면 §평소 톤·§철학 후보로 한 줄.
- 추출한 dominant 색·둥글기 결을 §철학 1줄 후보로 ("절제된 모노 결" / "따뜻한 파스텔" 식).
- §레퍼런스·§피하고 싶은 디자인은 글로벌 디자이너 프로필이 있을 때만 거기서 시드 (§7 처리).
- 비어있는 차원이 남아도 *여기선 묻지 X* — 첫 작업 시 `aesthetic-guard`가 자연스레 1개만.
...
미학 컨텍스트(§철학·§레퍼런스·§피하고 싶은 디자인)는 `CLAUDE.project.md`에 *빈 슬롯*으로 남겨두고, 첫 작업(컴포넌트·페이지 생성) 시 `aesthetic-guard`가 자연스레 1개씩 묻고 채움.
...
**글로벌 디자이너 프로필 시드** — `~/.claude/CLAUDE.md` §디자이너 프로필 슬롯이 채워져 있으면, 코드에서 *추출 불가능한 차원*만 시드:
- §레퍼런스 → 글로벌 "좋아하는 디자인" 그대로
- §피하고 싶은 디자인 → 글로벌 "피하고 싶은 디자인" 그대로
- §톤 한 단어 → 글로벌 "톤 단어"가 있으면 후보, *코드 추출 톤과 충돌하면 코드 추출 우선* (이 프로젝트의 사실이 더 우선)

전역 슬롯이 비어 있거나 전역 자체가 없으면 시드 X — `aesthetic-guard`가 첫 작업 시 자연스레 묻기.
