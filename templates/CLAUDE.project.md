...
> 이 파일은 디자이너가 만드는 *각 프로젝트 루트*에 생성됩니다. `/kdesigner:프로젝트시작`이 placeholder(`{{...}}`)를 사용자 답변·자동 추출로 채웁니다.
>
...
> 프로젝트 루트에 `DESIGN.md`(`https://getdesign.md/` 표준)가 있으면 *1순위 토큰 소스*로 자동 인식. `/kdesigner:프로젝트시작`이 `./CLAUDE.md`에 `@DESIGN.md` import를 자동으로 박고, `aesthetic-guard`/`design-system-guard`도 그 파일을 단일 진실로 참조. DESIGN.md 갱신은 사용자가 직접.
...
### 레퍼런스
> 이 프로젝트가 *닮고 싶은* 화면·프로덕트 1-3개. 디자이너가 *원하면* 적는 자리 — 비어 있어도 `aesthetic-guard`가 본문 고정 클리셰 회피로 작동.
...
> 글로벌 회피와 별개로 *이 서비스에서 절대 안 쓰는 것*. `aesthetic-guard`가 작업 마무리 시 가볍게 묻고, 같은 결 2회+ 반복되면 전역으로 승격 권유.
...
> 값이 비어 있으면 `design-system-guard`가 첫 사용 시 묻고 채움.
...
<!-- kd:slot:source-repo -->
## 원본 레포 (자동 관리)
> `import-existing`이 전체 레포(A 분기) 가져올 때 자동 채움 · `design-sync`가 *마지막 sync* 갱신. 비 git 저장소·원본 없으면 비워둠.
...
## 디자인 모드 설정 (자동 관리)
> `import-existing`이 *운영 레포 변환(A 분기, §3-D)* 시 자동 채움. 변환 안 했으면 비워둠.
> 핵심: prod 경계 코드(`middleware*`·`app/api/**`·`lib/**` 등)는 손대지 않고, 인증·API stub을 *별도 모듈*에 격리해 *컴포넌트 import 경로 차원에서만* 분기. 격리만으로 못 막는 *디자이너 화면 앞 에러 노출*은 별도 차단 레이어가 흡수.
...
### 에러 노출 차단 레이어 셋업 상태
> 디자이너 화면 앞 *완충층*. `import-existing` §3-D §6에서 스택 추론 후 카테고리별 셋업 여부를 *방향 표현*으로 누적 (특정 라이브러리·함수명 X — 인계 시 개발자가 *무엇이 차단되고 있는지*만 한눈에).

- **페이지 단위 Error Boundary 자연 진입**: {{미셋업 / 디자인 모드일 때만 활성 (위치 1줄)}}
- **dev 시점 에러 표시 비활성** (풀스크린 오버레이·HMR 빨간 박스 등): {{미셋업 / 디자인 모드 때 노출 차단}}
- **데이터 fetching 결과의 silent fallback**: {{미셋업 / 해당 사항 없음 / wrapper 활성 (위치 1줄)}}
- **schema validation의 디자인 모드 분기**: {{미셋업 / 해당 사항 없음 / 실패 시 빈/기본 형태 폴백}}
- **셋업 통과 신호**: {{미확인 / 첫 페이지 진입에서 영어 스택트레이스 노출 없이 화면 도달 확인}}

<!-- kd:slot:share-policy -->
## 공유 정책 (자동 관리)
> `external-share`가 첫 외부 공유 시 묻고 자동 갱신. 사용자가 직접 손대지 X.
...
> `publishing-guard`의 *경계 정의* — 디자이너가 어디까지 손대도 안전한지를 *프로젝트별로*. `import-existing` A 분기에서 프레임워크 휴리스틱으로 시드, 본 레포 구조 다르면 *개발자가 직접 조정*. 비어있으면 hook이 Next.js 기준 기본값 적용.
...
> 위 패턴 감지 시 `publishing-guard`가 (a) 격리 OK / (b) 격리 부족 / (c) 진짜 prod 영향 3분류로 톤 차별화. (c) + 명시 동의 시 다음 인계 회차의 *⚠️ 디자인 외 변경 — 검토 필요* 섹션에 자동 기록.
...
> *자동 관리* — `import-existing`이 첫 생성(기존 프로젝트 일괄), `design-system-guard`가 신규/변경 감지 시 갱신. 사용자가 직접 손대지 X.
...
> *자동 관리* — `import-existing` §4.5(라우트 패턴 자동 탐지: Next.js App/Pages·Vite+React Router·Expo·SvelteKit·Astro 등 휴리스틱)가 첫 생성, `design-system-guard`가 신규 감지 시 갱신(계획).
...
- 컴포넌트 안에 *하드코딩 X* — props로 받기만
- 실제 API 응답 형식과 일치 (인계 시 `mock/` → 실제 fetch만 갈아끼우면 되도록)

예: `mock/cafes.ts`가 `[{ id, name, address, isQuiet }]` 형식이면, 실서비스 API도 같은 형식.
...
| `components/` | 디자이너 | 재사용 컴포넌트 — 디자이너 영역, 개발자 그대로 |
| API 라우트 | 개발자 | 백엔드 연결 |
| 인증·권한 | 개발자 | 로그인·세션 |
...
## 공유 이력
> `external-share`가 자동 갱신. 인계 시 `export-handoff`가 참조 — *이 디자인이 누구에게 언제 공유됐는지* 맥락을 개발자에게 함께 전달.
...
<!-- kd:slot:last-work -->
## 마지막 작업
> `fresh-session-guide`가 자동 갱신. 새 대화 시작 시 "이어서 작업하자" 단서.

---

## 페르소나·응답 스타일 (이 프로젝트 안에서의 행동 정의)

> 전역(`~/.claude/CLAUDE.md`)에도 동일 정의가 박혀있으면 중복은 자연스럽게 무시되니 안전. **차이는 적용 범위뿐** — 전역 켬은 모든 폴더, 프로젝트만 켬은 이 폴더. 디자이너 친화 톤·git 추상화·자동 회복은 어느 쪽이든 동일.

- 사용자는 한국 디자이너 — Figma 능숙, git/터미널 미숙
- **응답은 한국어로**. 영어 단독 노출 X
- **용어 한글 병기**: **한국어 의미** (`영어 원문`) — 변경 영향 1줄
...
- **결정 피로 최소화**: 한 사이클 질문 3회 상한, 모든 객관식에 "잘 모르겠어요 / 추천해주세요"
- **응답 끝 다음 행동 1개**: "이제 ~~하시겠어요?" + 트리거 자연어 1개 — 0개·2개 이상 X
