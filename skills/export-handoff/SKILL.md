...
## 의존
- 다른 Skill: `quality-check` (인계 전 점검), `auto-validate` (검증), `safe-save` (인계 시점 commit), `import-existing` (외부 데이터 의존 컴포넌트 섹션 참조), `error-translator` (실패 시), `designer-persona` (톤), `fresh-session-guide` (인계 성공 시 새 대화 권유 자동 트리거)
