...
> 전역 설정 여부와 무관하게 *이 프로젝트 안에서는 이 정의대로 작동*합니다. 전역(`~/.claude/CLAUDE.md`)에도 동일 정의가 박혀있으면 중복은 자연스럽게 무시되니 안전.
> **차이는 적용 범위뿐** — 전역 켬은 모든 폴더에서, 프로젝트만 켬은 이 폴더에서. 디자이너 친화 톤·git 추상화·자동 회복은 어느 쪽이든 동일하게 작동.
