...
| 완전 빈 디렉토리(`Glob`로 파일 0개에 가까움) | 분석할 대상이 아직 없는 상태. 사용자가 이 Skill에 들어왔다는 사실 자체가 *기존 코드를 가져올 결*이라는 신호 — 빈 폴더라는 환경 조건으로 새 디자인 결로 우회시키지 않는다. 어떤 종류의 입력을 옮길 수 있는지(전체 레포 / 디자인+페이지 추출분 / 디자인 시스템 단독 / 컴포넌트 일부) 방향만 안내하고 종료. 사용자가 코드를 옮긴 뒤 자연어 발화로 이 Skill에 다시 들어온다. | — |
