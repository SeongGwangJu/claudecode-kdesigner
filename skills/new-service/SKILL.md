...
#### 7.1 글로벌 디자이너 프로필 시드 (있으면)

`~/.claude/CLAUDE.md`에 §디자이너 프로필 슬롯이 채워져 있으면 *프로젝트 미학 슬롯의 시작점*으로 가져온다. 매 프로젝트마다 같은 질문 반복 X.

1. `Read ~/.claude/CLAUDE.md` → §디자이너 프로필 슬롯 추출 (`<!-- kd:designer-mode:start -->`~`end` 사이).
2. "좋아하는 디자인 / 레퍼런스" → `CLAUDE.project.md` §레퍼런스 슬롯에 시드.
3. "피하고 싶은 디자인" → `CLAUDE.project.md` §피하고 싶은 디자인 (이 프로젝트 한정) 슬롯에 시드.
4. "톤 단어"가 채워져 있으면 §톤 한 단어 슬롯에 시드.

전역 슬롯이 비어 있거나(전역 켤 때 인터뷰 "잘 모르겠어요" 선택) 전역 자체가 없으면(§3-B 분기) 시드 X — `aesthetic-guard`가 첫 작업 발동 시 자연스레 묻기.
