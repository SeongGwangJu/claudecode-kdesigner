...
description: 세션이 충분히 무거워졌고 큰 흐름의 휴지점일 때 디자이너에게 새 대화창으로 옮기길 *잔소리 없이 한 번* 권유 — CLAUDE.project.md 자동 로드 덕분에 컨텍스트는 그대로 이어진다는 안심이 핵심. 주력 트리거는 *시스템 자가 점검* (§1)으로 사용자가 직접 부르는 일은 드물다. (a) `export-handoff` 인계 사이클 마무리 직후 / (b) 양적·질적 게이트 둘 다 yes일 때 자가 점검 자동 / (c) 보조 자연어("대화 길어졌나?"·"이거 무거워"·"새 대화 열까?"·"토큰 너무 쓰는 거 아냐?") — 같은 세션에서 1회만.
model: inherit
---
