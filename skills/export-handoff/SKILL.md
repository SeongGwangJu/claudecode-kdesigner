...
`/kd:디자인초기설정`/`new-service`/`import-existing`/`safe-save`가 박아둔 *디자이너 모드 진입 시점 SHA*를 읽어 그 시점부터 HEAD까지의 변경분만 추린다.
...
`import-existing` Skill이 만든 `CLAUDE.project.md`의 `## 외부 데이터 의존 컴포넌트` 섹션은 *기존 컴포넌트의 의존*을 이미 적어뒀음(인계 시점에 다시 적을 필요 X). 이 Skill은 *이번 사이클에 새로 도입한 데이터 hook*만 추가로 표기한다 — 중복 방지 + 개발자 시선이 *바뀐 부분*에 모이게.

스캔 대상: §2.0의 CHANGED_FILES 안 컴포넌트 파일만.

스캔 패턴:
- `useAuth`, `useSession`, `useQuery`, `useSWR`, `useMutation` 등 데이터 훅 사용
- 직접 `fetch()` / `axios()` 호출

fallback 분기(CHANGED_FILES = 전체)에서는 기존 동작대로 전체 스캔하되, `CLAUDE.project.md`에 이미 있는 항목은 중복 표기 X.
