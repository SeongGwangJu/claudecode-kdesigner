...
- §레퍼런스·§피하고 싶은 표현은 글로벌 디자이너 프로필이 있을 때만 거기서 시드 (§7 처리).
...
미학 컨텍스트(§철학·§레퍼런스·§피하고 싶은 표현)는 `CLAUDE.project.md`에 *빈 슬롯*으로 남겨두고, 첫 작업(컴포넌트·페이지 생성) 시 `aesthetic-guard`가 자연스레 1개씩 묻고 채움.
