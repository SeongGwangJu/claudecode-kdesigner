---
description: |
  새 서비스 0→1 셋업. 사용자가 새로 만들겠다는 결을 표현했을 때 발동한다. 추천 스택(웹: Tailwind+shadcn/ui+Next.js+TypeScript / 앱: Expo+NativeWind)으로 폴더 구조(`asset/`, `mock/`, `components/`) 생성, `CLAUDE.project.md` placeholder 시드, `package.json` scripts(`lint`/`typecheck`) 보장. 셋업 후 디자이너가 첫 화면을 만질 수 있는 상태까지 도달.
  ...
  사용 시점: 사용자가 새로 시작하겠다는 의도를 표현한 직후. 빈 디렉토리라는 환경 조건만으로는 충분하지 않다 — 빈 폴더에도 기존 코드를 가져올 결이 있어 의도 확인이 앞선다 (`/kdesigner:프로젝트시작` §5.1). 기존 코드가 이미 있는 디렉토리에서는 `import-existing`이 더 자연스러운 결.
...
### 발동
- 사용자가 새로 시작하겠다는 결을 표현하는 자연어 발화
- `/kdesigner:프로젝트시작` §5.1 의도 확인 결과 새 디자인 쪽으로 결이 잡힌 경우

### 발동이 망설여지는 결
- 디렉토리가 비어있다는 환경 조건 자체만으로 발동하면 *기존 코드를 풀어놓을 예정*이던 사용자의 결을 가로채게 된다. 환경 조건은 신호일 뿐 트리거가 아니다 — 의도 표현이 앞선다.
- 기존 코드가 이미 있는 디렉토리에서는 `import-existing`의 결이 자연스럽다.
